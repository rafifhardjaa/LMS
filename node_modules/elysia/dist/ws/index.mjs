...
function isInvalidResponse(validator, data) {
  return validator ? validator.provider === "standard" ? validator.schema["~standard"].validate(data).issues : validator.Check(data) === !1 : !1;
}
const websocket = {
  open(ws) {
    ws.data.open?.(ws);
...
class ElysiaWS {
   ...
   */
  send(data, compress) {
    return Buffer.isBuffer(data) ? this.raw.send(data, compress) : isInvalidResponse(this.validator, data) ? this.raw.send(
   ...
   */
  ping(data) {
    return Buffer.isBuffer(data) ? this.raw.ping(data) : isInvalidResponse(this.validator, data) ? this.raw.send(
      new ValidationError("message", this.validator, data).message
   ...
   */
  pong(data) {
    return Buffer.isBuffer(data) ? this.raw.pong(data) : isInvalidResponse(this.validator, data) ? this.raw.send(
      new ValidationError("message", this.validator, data).message
  ...
  publish(topic, data, compress) {
    return Buffer.isBuffer(data) ? this.raw.publish(
      ...
      data,
      compress
    ) : isInvalidResponse(this.validator, data) ? this.raw.send(
      new ValidationError("message", this.validator, data).message
