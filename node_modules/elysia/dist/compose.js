...
`;
  ...
  if (hasQuery) {
    ...
    if (validator.query?.schema) {
      const schema = (0, import_schema.unwrapImportSchema)(validator.query?.schema), properties = (0, import_schema.getSchemaProperties)(schema);
      if (properties)
    ...
    if (validator.headers) {
      if (validator.headers.hasDefault)
        for (const [key, value] of Object.entries(
          ...
          parsed !== void 0 && (fnLiteral += `c.headers[${JSON.stringify(key)}]??=${parsed}
`);
        }
    ...
    if (validator.params) {
      if (validator.params.hasDefault)
        for (const [key, value] of Object.entries(
          ...
          const parsed = typeof value == "object" || typeof value == "string" ? JSON.stringify(value) : value;
          parsed !== void 0 && (fnLiteral += `c.params[${JSON.stringify(key)}]??=${parsed}
`);
        }
    ...
    if (validator.query) {
      if (import_typebox.Kind in validator.query?.schema && validator.query.hasDefault)
        for (const [key, value] of Object.entries(
          ...
          const parsed = typeof value == "object" || typeof value == "string" ? JSON.stringify(value) : value;
          parsed !== void 0 && (fnLiteral += `if(c.query[${JSON.stringify(key)}]===undefined)c.query[${JSON.stringify(key)}]=${parsed}
`);
        }
      fnLiteral += composeCleaner({
        name: "c.query",
...
`,
        ...
        for (let i = 0; i < iterator.length; i++) {
          ...
          if ((0, import_schema.hasType)("File", type) || (0, import_schema.hasType)("Files", type)) {
            ...
            if (candidate) {
              const isFirst = fileUnions.length === 0, properties = (0, import_schema.getSchemaProperties)(candidate.schema) ?? (0, import_schema.getSchemaProperties)(type);
...
`), validator2 += "}", fnLiteral += validator2, fileUnions.push(candidate));
        ...
        let validateFile = "";
        const bodyProperties = (0, import_schema.getSchemaProperties)(
          (0, import_schema.unwrapImportSchema)(validator.body.schema)
        );
        let i = 0;
        if (bodyProperties)
...
`) : validator.cookie?.schema?.noValidate !== !0 && (fnLiteral += "if(validator.cookie.Check(cookieValue)===false){" + validation.validate("cookie", "cookieValue") + "}", validator.cookie.hasTransform && (fnLiteral += coerceTransformDecodeError(
