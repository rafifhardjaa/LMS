...
var import_value = require("@sinclair/typebox/value"), import_cookies = require('./cookies.js'), import_error = require('./error.js'), import_parse_query = require('./parse-query.js'), import_schema = require('./schema.js'), import_utils = require('./utils.js');
const ARRAY_INDEX_REGEX = /^(.+)\[(\d+)\]$/, DANGEROUS_KEYS = /* @__PURE__ */ new Set(["__proto__", "constructor", "prototype"]), isDangerousKey = (key) => {
  ...
  const properties = (0, import_schema.getSchemaProperties)(schema);
  if (properties)
    ...
    try {
        ...
        if (content)
          switch (content) {
            ...
            case "multipart/form-data": {
              body = {};
              const form = await request.formData(), grouped = /* @__PURE__ */ new Map();
              form.forEach((v, k) => {
      ...
      if (validator) {
        if (headerValidator) {
          ...
          else if (validator.headers.Check(_header) === !1)
            throw new import_error.ValidationError(
              "header",
              validator.headers,
              _header
      ...
      if (response instanceof Promise && (response = await response), hooks.afterHandle?.length) {
        ...
        for (let i = 0; i < hooks.afterHandle.length; i++) {
          ...
          if (responseValidator?.provider === "standard")
            response2 = await validateStandardSchema(
              "response",
              responseValidator,
              response2
            );
          else if (responseValidator?.Check(response2) === !1)
            if (responseValidator?.Clean)
              try {
                ...
                throw error instanceof import_error.ValidationError ? error : new import_error.ValidationError(
                  "response",
                  responseValidator,
                  response2
                );
        ...
        else if (responseValidator?.Check(response) === !1)
          if (responseValidator?.Clean)
            try {
      ...
      if (context.set.cookie && cookieMeta?.sign) {
        ...
        if (cookieMeta.sign === !0) {
          ...
          const properties = (0, import_schema.getSchemaProperties)(validator?.cookie?.schema);
          if (secret)
            for (const name of cookieMeta.sign)
              !properties || !(name in properties) || context.set.cookie[name]?.value && (context.set.cookie[name].value = await (0, import_utils.signCookie)(
