...
}, MAX_ERRORS = 64, takeErrors = (iterator, limit = MAX_ERRORS) => {
  const errors = [];
  for (const error of iterator) {
    if (!error) continue;
    const mapped = mapValueError(error);
    if (mapped && errors.push(mapped), errors.length >= limit) break;
  }
  return errors;
};
...
class ValidationError extends Error {
    ...
    if (
      ...
      customError = error?.schema?.message || error?.schema?.error !== void 0 ? typeof error.schema.error == "function" ? error.schema.error(
        isProduction && !allowUnsafeValidationDetails ? {
          ...
          found: value,
          expected,
          errors: "Errors" in validator ? takeErrors(
            validator.Errors(
              value
            )
          ) : takeErrors(
      ...
      }) : message = JSON.stringify(
        {
          ...
          expected,
          found: value,
          errors: "Errors" in validator ? takeErrors(validator.Errors(value)) : takeErrors(import_value.Value.Errors(validator, value))
        },
        null,
        2
      );
  ...
  get all() {
    return (
      ...
      ))["~standard"].validate(this.value).issues?.slice(0, MAX_ERRORS).map((issue) => ({
        summary: issue.message,
        path: issue.path?.join(".") || "root",
        message: issue.message,
        value: this.value
      })) || [] : "Errors" in this.validator ? takeErrors(this.validator.Errors(this.value)) : (
        // @ts-ignore
        takeErrors(import_value.Value.Errors(this.validator, this.value))
      )
    );
  }
